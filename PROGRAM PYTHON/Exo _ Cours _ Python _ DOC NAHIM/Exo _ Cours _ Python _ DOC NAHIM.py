...
while longueur == 0:
    try:
        longueur_str = input(
            "Entrer la longueur du solide en mètre, (Un nombre svp)")
        longueur = int(longueur_str)
    except:
        print("Veuiller entrer un nombre pour la longueur")
print(
    f"Génial, Votre solide à une longueur de {longueur} mètres. Passons à la prochaine étape!")
print()
...
while largeur == 0:
    try:
        largeur_str = input(
            "Entrer la largeur du solide en mètre, (Un nombre svp)")
        largeur = int(largeur_str)
    except:
        print("Veuiller entrer un nombre pour la largeur")
print(
    f"Génial, Votre solide à une largeur de {largeur} mètres. Passons à la prochaine étape!")
print()
...
while hauteur == 0:
    try:
        hauteur_str = input(
            "Entrer la hauteur du solide en mètre, (Un nombre svp)")
...
def demander_seconde():
    ...
    while seconde == 0:
        try:
            seconde_str = input(
                "Veuillez entrer le nombre de seconde que vous voulez convertir : ")
...
def demander_angle():
    ...
    while angle == 0:
        try:
            angle_str = input(
                "Veuillez saisir l'angle que vous voulez convertir : ")
            angle = int(angle_str)
        except:
...
t3 = [t2[0], t1[0], t2[1], t1[1], t2[2], t1[2], t2[3], t1[3], t2[4], t1[4], t2[5], t1[5],
    t2[6], t1[6], t2[7], t1[7], t2[8], t1[8], t2[9], t1[9], t2[10], t1[10], t2[11], t1[11]]
print(t3)
"""
...
def demander_mot():
    mot = input(
        "Veuillez entrer un mot (Cliquez sur entrer si vous n'avez plus de mot) : ")
    liste.append(mot)
    while mot != "":
        mot = input(
            "Veuillez entrer un mot (Cliquez sur entrer si vous n'avez plus de mot) : ")
...
while vitesse == 0:
    try:
        vitesse_str = input(
            "Veuillez entrer la vitesse que vous voulez convrtir : ")
...
while longueur == 0:
    try:
        ...
        print("Veuiller entrer un nombre pour la longueur")
print(
    f"Génial, Votre pendule à une longueur de {longueur} mètres. Passons à la prochaine étape!")
print()
...
print(
    f"La somme des nombres multiples de 3 ou de 5 entre {nbr1} et {nbr2} est : {total_ou} ")
print()
print(
    f"La somme des nombres multiples de 3 et de 5 entre {nbr1} et {nbr2} est : {total_et} ")
"""
...
while note >= 0:
    try:
        note_str = input(
            f"Veuillez saisir une note (Saisissez un nombre négatif pour arrêter)  : ")
        note = int(note_str)
        if note >= 0 :
            liste.append(note)
    except:
...
print(f"La moyenne des notes est {M}")
"""
...
while c < 20:
    F = 6.67 * (10**-11) * (10**8)/(d**2)
    print(
        f"La force de gravitation s'exerçant entre deux masses de 10 000 kg séparées par une distance de {d} mètres est : {F} newton")
    d = d * 2
    c = c +1
"""
...
tex1 = Label(fen1, text='Bonjour tout le monde !',
             fg='red') # Instanciation du module Label()
# Sert à l'encapsulation de l'objet et l'adaptation de la géométrie de la fenêtre
tex1.pack()
# Instanciation du module Label()
...
def evaluer(event):
 # get() --> Extraction du contenu de entree
 chaine.configure(text=f"Resultat = {str(eval(entree.get()))}")
                                                                # eval() --> Evaluation
# ----- Programme principal : -----
fenetre = Tk()
# Création de la zone pour recevoir le formules saisies par l'utilisateur
entree = Entry(fenetre)
# Lier l'événement <pression sur la touche Return> à l'objet <entree>,
entree.bind("<Return>", evaluer)
                                        # le gestionnaire de cet événement étant la fonction <evaluer>
# Création de la chaine à configurer pour afficher le résultat de l'opération
...
def pointeur(event):
    chaine.configure(
        text=f"Clic detecte en X ={str(event.x)}, Y ={str(event.y)}")
fen = Tk()
...
# CodeCours 8 :  Animation automatique - Récursivité
...
# Exercices
# Exo 65  :  .Dans la fonction start_it(), supprimez l'instruction if flag == 0: (et l'indentation des deux lignes suivantes). Que se
# passe-t-il ? (Cliquez plusieurs fois sur le bouton « Démarrer ».)
# Tâchez d'exprimer le plus clairement possible votre explication des faits observés.


# Exo 66 :  .Modifiez le programme de telle façon que la balle change de couleur à chaque « virage ».


# Exo 67  :  .Modifiez le programme de telle façon que la balle effectue des mouvements obliques comme une bille de billard qui
# rebondit sur les bandes (« en zig-zag »).


# Exo 68  :  .Modifiez le programme de manière à obtenir d'autres mouvements. Tâchez par exemple d'obtenir un mouvement
# circulaire (comme dans les exercices de la page ).


# Exo 69  :  .Modifiez ce programme, ou bien écrivez-en un autre similaire, de manière à simuler le mouvement d'une balle qui
# tombe (sous l'effet de la pesanteur), et rebondit sur le sol. Attention : il s'agit cette fois de mouvements accélérés !


# Exo 70  :  .À partir des scripts précédents, vous pouvez à présent écrire un programme de jeu fonctionnant de la manière
# suivante : une balle se déplace au hasard sur un canevas, à vitesse faible. Le joueur doit essayer de cliquer sur cette
# balle à l'aide de la souris. S'il y arrive, il gagne un point, mais la balle se déplace désormais un peu plus vite, et ainsi
# de suite. Arrêter le jeu après un certain nombre de clics et afficher le score atteint.
# Variante du jeu précédent : chaque fois que le joueur parvient à « l'attraper », la balle devient plus petite (elle peut
# également changer de couleur).


# Exo 71  :  .Écrivez un programme dans lequel évoluent plusieurs balles de couleurs différentes, qui rebondissent les unes sur
# les autres ainsi que sur les parois.


# Exo 72  :  .Perfectionnez le jeu des précédents exercices en y intégrant l'algorithme ci-dessus. Il s'agit à présent pour le joueur
# de cliquer seulement sur la balle rouge. Un clic erroné (sur une balle d'une autre couleur) lui fait perdre des points.

# Exo 73  :  .Écrivez un programme qui simule le mouvement de deux planètes tournant autour du soleil sur des orbites circulaires
# différentes (ou deux électrons tournant autour d'un noyau d'atome...).


# Exo 74  :  .Écrivez un programme pour le jeu du serpent : un « serpent » (constitué en fait d'une courte ligne de carrés) se
# déplace sur le canevas dans l'une des 4 directions : droite, gauche, haut, bas. Le joueur peut à tout moment changer
# la direction suivie par le serpent à l'aide des touches fléchées du clavier. Sur le canevas se trouvent également des
# « proies » (des petits cercles fixes disposés au hasard). Il faut diriger le serpent de manière à ce qu'il « mange »
# les proies sans arriver en contact avec les bords du canevas. À chaque fois qu'une proie est mangée, le serpent
# s'allonge d'un carré, le joueur gagne un point, et une nouvelle proie apparaît ailleurs. La partie s'arrête lorsque le
# serpent touche l'une des parois, ou lorsqu'il a atteint une certaine taille.
